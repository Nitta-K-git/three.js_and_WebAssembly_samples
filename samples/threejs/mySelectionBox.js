...
var SelectionBox = (function () {
	...
	// 点群データの頂点だけピッキングする
	// すべての頂点でcontainsPointして結果をフラグのリスト形式で返す
	SelectionBox.prototype.searchChildInFrustum = function (frustum, object) {
		if (object.isPoints) {
			let p = new Vector3();
			for(let i=0; i<object.geometry.vertices.length; i++){
				p.copy(object.geometry.vertices[i]);
				p.applyMatrix4(object.matrixWorld); // groupになっているときの変換行列
				if (frustum.containsPoint(p)) {
					this.collection.push(i); // indexをpush
				}
			}
